    ...
    IOptions<StudentCredentialEmailOptions> options) : IStudentCredentialEmailSender
{
    ...
    private void ValidateConfiguration()
    {
        ...
        {
            throw new InvalidOperationException("StudentCredentialEmail:Host must be configured before sending student credentials.");
        }

        if (string.IsNullOrWhiteSpace(_options.SenderEmail))
        {
            throw new InvalidOperationException("StudentCredentialEmail:SenderEmail must be configured before sending student credentials.");
        }
    }
}
